...
import { CloudUpload as CloudUploadIcon, Download as DownloadIcon, CheckCircle as CheckCircleIcon, Cancel as CancelIcon, Add as AddIcon } from '@mui/icons-material';
...
function ProjectImportPage() {
  ...
  const handleUploadForPreview = async () => {
    if (!checkUserPrivilege(user, 'project.create')) {
      setSnackbar({ open: true, message: 'You do not have permission to import projects.', severity: 'error' });
      return;
    }
    if (!selectedFile) {
    ...
    try {
      const response = await apiService.projects.previewProjectImport(formData);
  ...
  const handleConfirmImport = async () => {
    if (!checkUserPrivilege(user, 'project.create')) {
      setSnackbar({ open: true, message: 'You do not have permission to confirm project import.', severity: 'error' });
      return;
    }
    ...
    setLoading(true);
    setSnackbar({ open: true, message: 'Confirming import and saving projects...', severity: 'info' });
    setImportReport(null);

    try {
      const response = await apiService.projects.confirmProjectImport({ dataToImport: fullParsedData });
      setSnackbar({ open: true, message: response.message, severity: 'success' });
      ...
      // Navigate back to projects page after successful import
      setTimeout(() => {
        navigate('/projects');
      }, 2000);
    } catch (err) {
  ...
  const handleDownloadTemplate = async () => {
    setLoading(true);
    try {
      const response = await apiService.projects.downloadProjectTemplate();
  ...
  const isUploadButtonDisabled = !selectedFile || loading || !checkUserPrivilege(user, 'project.create');

  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h4" gutterBottom>Import Projects Data</Typography>
            ...
            {previewData && (
              <Box sx={{ display: 'flex', gap: 1 }}>
                <Button
                  ...
                  disabled={loading || !checkUserPrivilege(user, 'project.create')}
                  fullWidth
                >
