...
import { Close, Public, AttachMoney, Assessment, CheckCircle } from '@mui/icons-material';
import ProjectDetailTable from '../tables/ProjectDetailTable';
import regionalService from '../../api/regionalService';

const CountyProjectsModal = ({ open, onClose, county }) => {
    ...
    const fetchCountyProjects = async () => {
        setIsLoading(true);
        setError(null);
        
        try {
            const response = await regionalService.getProjectsByCounty(county.county);
            ...
            console.error('Error fetching county projects:', error);
            setProjects(generateMockProjects());
            setIsUsingMockData(true);
        } finally {
            setIsLoading(false);
    ...
    const generateMockProjects = () => {
        ...
        for (let i = 0; i < 8; i++) {
            mockProjects.push({
                ...
                county: county?.county || 'Sample County',
                subCounty: `Sub-County ${i + 1}`,
                ward: `Ward ${i + 1}`,
                village: `Village ${i + 1}`,
                ...
                allocatedBudget: (Math.random() * 50000000 + 10000000).toFixed(2),
                contractSum: (Math.random() * 45000000 + 9000000).toFixed(2),
                amountPaid: (Math.random() * 40000000 + 5000000).toFixed(2),
                absorptionRate: (Math.random() * 100).toFixed(2),
                objective: `Improve infrastructure in ${county?.county || 'Sample County'}`,
    ...
    const stats = calculateCountyStats();
...
export default CountyProjectsModal;
