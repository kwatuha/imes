...
import { Close, Map, AttachMoney, Assessment, CheckCircle } from '@mui/icons-material';
import ProjectDetailTable from '../tables/ProjectDetailTable';
import regionalService from '../../api/regionalService';

const WardProjectsModal = ({ open, onClose, ward }) => {
    ...
    const fetchWardProjects = async () => {
        ...
        try {
            const response = await regionalService.getProjectsByWard(ward.ward);
    ...
    const generateMockProjects = () => {
        ...
        for (let i = 0; i < 4; i++) {
            mockProjects.push({
                ...
                county: ward?.county || 'Sample County',
                subCounty: ward?.subCounty || 'Sample Sub-County',
                ward: ward?.ward || 'Sample Ward',
                village: `Village ${i + 1}`,
                ...
                allocatedBudget: (Math.random() * 15000000 + 2000000).toFixed(2),
                contractSum: (Math.random() * 14000000 + 1500000).toFixed(2),
                amountPaid: (Math.random() * 12000000 + 1000000).toFixed(2),
                absorptionRate: (Math.random() * 100).toFixed(2),
                objective: `Improve infrastructure in ${ward?.ward || 'Sample Ward'}`,
        ...
        >
            <DialogTitle sx={{ 
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <Map sx={{ fontSize: '1.5rem' }} />
                    <Typography variant="h6" sx={{ fontWeight: 'bold' }}>
                        {ward?.ward || 'Ward'} Projects
                    </Typography>
                </Box>
                <Button
                    onClick={onClose}
