...
async function addMissingFinancialYears() {
  ...
  try {
    ...
    const [existing] = await connection.query(
      'SELECT finYearId, finYearName, startDate, endDate FROM kemri_financialyears WHERE voided = 0'
    );
      ...
      try {
        ...
        if (matching.length > 0) {
          ...
          const [existingByName] = await connection.query(
            `SELECT finYearId, finYearName FROM kemri_financialyears 
          ...
          if (existingByName.length > 0) {
            ...
            try {
              await connection.query(
                `INSERT INTO kemri_financialyears (finYearName, startDate, endDate, remarks, voided, userId) 
                 VALUES (?, ?, ?, ?, 0, 1)`,
