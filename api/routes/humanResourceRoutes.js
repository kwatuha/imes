...
router.get('/employees', auth, privilege(['employee.read_all']), async (req, res) => {
    try {
        const sql = `
            ...
            FROM kemri_staff s
            LEFT JOIN kemri_departments d ON s.departmentId = d.departmentId AND d.voided = 0
            LEFT JOIN kemri_job_groups jg ON s.jobGroupId = jg.id
            WHERE s.voided = 0
        `;
...
router.post('/export/employees-excel', auth, privilege(['employee.read_all']), async (req, res) => {
    try {
        ...
        const sql = `
            ...
            LEFT JOIN kemri_departments d ON s.departmentId = d.departmentId AND d.voided = 0
            LEFT JOIN kemri_job_groups jg ON s.jobGroupId = jg.id
            LEFT JOIN kemri_staff m ON s.managerId = m.staffId
            WHERE s.voided = 0
...
router.get('/employees/:id/360', auth, privilege(['employee.read_all', 'employee.read_360']), async (req, res) => {
    ...
    try {
        ...
        const dataPromises = [
            db.query('SELECT * FROM kemri_employee_performance WHERE staffId = ? AND voided = 0 ORDER BY reviewDate DESC', [id]),
            db.query('SELECT * FROM kemri_employee_compensation WHERE staffId = ? AND voided = 0', [id]),
            db.query('SELECT * FROM kemri_employee_training WHERE staffId = ? AND voided = 0 ORDER BY completionDate DESC', [id]),
            db.query('SELECT * FROM kemri_employee_disciplinary WHERE staffId = ? AND voided = 0 ORDER BY actionDate DESC', [id]),
            db.query('SELECT * FROM kemri_employee_contracts WHERE staffId = ? AND voided = 0 ORDER BY contractStartDate DESC', [id]),
            db.query('SELECT * FROM kemri_employee_retirements WHERE staffId = ? AND voided = 0', [id]),
            db.query('SELECT * FROM kemri_employee_loans WHERE staffId = ? AND voided = 0', [id]),
            db.query('SELECT * FROM kemri_monthly_payroll WHERE staffId = ? AND voided = 0 ORDER BY payPeriod DESC', [id]),
            db.query('SELECT * FROM kemri_employee_dependants WHERE staffId = ? AND voided = 0', [id]),
            db.query('SELECT * FROM kemri_employee_terminations WHERE staffId = ? AND voided = 0', [id]),
            db.query('SELECT * FROM kemri_employee_bank_details WHERE staffId = ? AND voided = 0', [id]),
            db.query('SELECT * FROM kemri_employee_memberships WHERE staffId = ? AND voided = 0', [id]),
            db.query('SELECT * FROM kemri_employee_benefits WHERE staffId = ? AND voided = 0', [id]),
            db.query('SELECT * FROM kemri_assigned_assets WHERE staffId = ? AND voided = 0', [id]),
            db.query('SELECT * FROM kemri_employee_promotions WHERE staffId = ? AND voided = 0 ORDER BY promotionDate DESC', [id]),
            db.query('SELECT * FROM kemri_employee_project_assignments WHERE staffId = ? AND voided = 0', [id]),
            db.query(`
                SELECT
                    la.*,
            ...
            `, [id]),
            db.query('SELECT * FROM kemri_job_groups WHERE voided = 0')
        ];
...
router.post('/employees/performance', auth, privilege(['employee.performance.create']), async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO kemri_employee_performance (staffId, reviewDate, reviewScore, comments, reviewerId) VALUES (?, ?, ?, ?, ?)', [staffId, formatDate(reviewDate), reviewScore, comments, reviewerId]);
...
router.get('/leave-types', async (req, res) => {
    try {
        const [rows] = await db.query('SELECT * FROM kemri_leave_types WHERE voided = 0');
        res.json(rows);
    } catch (err) {
...
router.post('/leave-types', auth, privilege(['leave.type.create']), async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO kemri_leave_types (name, description, numberOfDays, userId) VALUES (?, ?, ?, ?)', [name, description, numberOfDays, userId]);
...
router.post('/attendance/check-in', auth, privilege(['attendance.create']), async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO kemri_attendance (staffId, date, checkInTime, userId) VALUES (?, CURDATE(), NOW(), ?)', [staffId, userId]);
...
router.put('/attendance/check-out/:id', auth, privilege(['attendance.create']), async (req, res) => {
    ...
    try {
        const [result] = await db.query('UPDATE kemri_attendance SET checkOutTime = NOW(), userId = ?, updatedAt = NOW() WHERE id = ?', [userId, id]);
        if (result.affectedRows === 0) {
...
router.post('/employee-compensation', auth, privilege(['compensation.create']), async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO kemri_employee_compensation (staffId, baseSalary, allowances, bonuses, bankName, accountNumber, payFrequency, userId) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [staffId, baseSalary, allowances, bonuses, bankName, accountNumber, payFrequency, userId]);
...
router.put('/employee-compensation/:id', auth, privilege(['compensation.update']), async (req, res) => {
    ...
    const { staffId, baseSalary, allowances, bonuses, bankName, accountNumber, payFrequency, userId } = req.body;
    try {
        const [result] = await db.query('UPDATE kemri_employee_compensation SET staffId = ?, baseSalary = ?, allowances = ?, bonuses = ?, bankName = ?, accountNumber = ?, payFrequency = ?, userId = ?, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [staffId, baseSalary, allowances, bonuses, bankName, accountNumber, payFrequency, userId, id]);
        if (result.affectedRows === 0) {
...
router.delete('/employee-compensation/:id', auth, privilege(['compensation.delete']), async (req, res) => {
    const { id } = req.params;
    try {
        const [result] = await db.query('UPDATE kemri_employee_compensation SET voided = 1, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Compensation record not found.' });
        }
...
router.post('/employee-training', auth, privilege(['training.create']), async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO kemri_employee_training (staffId, courseName, institution, certificationName, completionDate, expiryDate, userId) VALUES (?, ?, ?, ?, ?, ?, ?)', [staffId, courseName, institution, certificationName, formatDate(completionDate), formatDate(expiryDate), userId]);
...
router.put('/employee-training/:id', auth, privilege(['training.update']), async (req, res) => {
    ...
    const { staffId, courseName, institution, certificationName, completionDate, expiryDate, userId } = req.body;
    try {
        const [result] = await db.query('UPDATE kemri_employee_training SET staffId = ?, courseName = ?, institution = ?, certificationName = ?, completionDate = ?, expiryDate = ?, userId = ?, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [staffId, courseName, institution, certificationName, formatDate(completionDate), formatDate(expiryDate), userId, id]);
        if (result.affectedRows === 0) {
...
router.delete('/employee-training/:id', auth, privilege(['training.delete']), async (req, res) => {
    const { id } = req.params;
    const sql = 'UPDATE kemri_employee_training SET voided = 1, updatedAt = CURRENT_TIMESTAMP WHERE id = ?';
    try {
        const [result] = await db.query(sql, [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Training record not found.' });
...
router.get('/job-groups', auth, privilege(['job_group.read_all']), async (req, res) => {
    try {
        const [rows] = await db.query('SELECT * FROM kemri_job_groups WHERE voided = 0');
        res.json(rows);
    } catch (err) {
...
router.post('/job-groups', auth, privilege(['job_group.create']), async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO kemri_job_groups (groupName, salaryScale, description, userId) VALUES (?, ?, ?, ?)', [groupName, salaryScale, description, userId]);
...
router.put('/job-groups/:id', auth, privilege(['job_group.update']), async (req, res) => {
    ...
    const { groupName, salaryScale, description, userId } = req.body;
    try {
        const [result] = await db.query('UPDATE kemri_job_groups SET groupName = ?, salaryScale = ?, description = ?, userId = ? WHERE id = ?', [groupName, salaryScale, description, userId, id]);
        if (result.affectedRows === 0) {
...
router.delete('/job-groups/:id', auth, privilege(['job_group.delete']), async (req, res) => {
    const { id } = req.params;
    try {
        const [result] = await db.query('UPDATE kemri_job_groups SET voided = 1 WHERE id = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Job group not found.' });
        }
...
router.post('/employee-promotions', auth, privilege(['promotion.create']), async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO kemri_employee_promotions (staffId, oldJobGroupId, newJobGroupId, promotionDate, comments, userId) VALUES (?, ?, ?, ?, ?, ?)', [staffId, oldJobGroupId, newJobGroupId, formatDate(promotionDate), comments, userId]);
...
router.put('/employee-promotions/:id', auth, privilege(['promotion.update']), async (req, res) => {
    ...
    const { staffId, oldJobGroupId, newJobGroupId, promotionDate, comments, userId } = req.body;
    try {
        const [result] = await db.query('UPDATE kemri_employee_promotions SET staffId = ?, oldJobGroupId = ?, newJobGroupId = ?, promotionDate = ?, comments = ?, userId = ? WHERE id = ?', [staffId, oldJobGroupId, newJobGroupId, formatDate(promotionDate), comments, userId, id]);
        if (result.affectedRows === 0) {
...
router.delete('/employee-promotions/:id', auth, privilege(['promotion.delete']), async (req, res) => {
    const { id } = req.params;
    try {
        const [result] = await db.query('UPDATE kemri_employee_promotions SET voided = 1 WHERE id = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Promotion record not found.' });
        }
...
router.post('/employee-disciplinary', auth, privilege(['disciplinary.create']), async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO kemri_employee_disciplinary (staffId, actionType, actionDate, reason, comments, userId) VALUES (?, ?, ?, ?, ?, ?)', [staffId, actionType, formatDate(actionDate), reason, comments, userId]);
...
router.put('/employee-disciplinary/:id', auth, privilege(['disciplinary.update']), async (req, res) => {
    ...
    const { staffId, actionType, actionDate, reason, comments, userId } = req.body;
    try {
        const [result] = await db.query('UPDATE kemri_employee_disciplinary SET staffId = ?, actionType = ?, actionDate = ?, reason = ?, comments = ?, userId = ? WHERE id = ?', [staffId, actionType, formatDate(actionDate), reason, comments, userId, id]);
        if (result.affectedRows === 0) {
...
router.delete('/employee-disciplinary/:id', auth, privilege(['disciplinary.delete']), async (req, res) => {
    const { id } = req.params;
    try {
        const [result] = await db.query('UPDATE kemri_employee_disciplinary SET voided = 1 WHERE id = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Disciplinary record not found.' });
        }
...
router.post('/employee-contracts', auth, privilege(['contracts.create']), async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO kemri_employee_contracts (staffId, contractType, contractStartDate, contractEndDate, status, userId) VALUES (?, ?, ?, ?, ?, ?)', [staffId, contractType, formatDate(contractStartDate), formatDate(contractEndDate), status, userId]);
...
router.put('/employee-contracts/:id', auth, privilege(['contracts.update']), async (req, res) => {
    ...
    const { staffId, contractType, contractStartDate, contractEndDate, status, userId } = req.body;
    try {
        const [result] = await db.query('UPDATE kemri_employee_contracts SET staffId = ?, contractType = ?, contractStartDate = ?, contractEndDate = ?, status = ?, userId = ? WHERE id = ?', [staffId, contractType, formatDate(contractStartDate), formatDate(contractEndDate), status, userId, id]);
        if (result.affectedRows === 0) {
...
router.delete('/employee-contracts/:id', auth, privilege(['contracts.delete']), async (req, res) => {
    const { id } = req.params;
    try {
        const [result] = await db.query('UPDATE kemri_employee_contracts SET voided = 1, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Contract not found.' });
        }
...
router.post('/employee-retirements', auth, privilege(['retirements.create']), async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO kemri_employee_retirements (staffId, retirementDate, retirementType, comments, userId) VALUES (?, ?, ?, ?, ?)', [staffId, formatDate(retirementDate), retirementType, comments, userId]);
...
router.put('/employee-retirements/:id', auth, privilege(['retirements.update']), async (req, res) => {
    ...
    const { staffId, retirementDate, retirementType, comments, userId } = req.body;
    try {
        const [result] = await db.query('UPDATE kemri_employee_retirements SET staffId = ?, retirementDate = ?, retirementType = ?, comments = ?, userId = ? WHERE id = ?', [staffId, formatDate(retirementDate), retirementType, comments, userId, id]);
        if (result.affectedRows === 0) {
...
router.delete('/employee-retirements/:id', auth, privilege(['retirements.delete']), async (req, res) => {
    const { id } = req.params;
    try {
        const [result] = await db.query('UPDATE kemri_employee_retirements SET voided = 1 WHERE id = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Retirement record not found.' });
        }
...
router.post('/employee-loans', auth, privilege(['loans.create']), async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO kemri_employee_loans (staffId, loanAmount, loanDate, status, repaymentSchedule, userId) VALUES (?, ?, ?, ?, ?, ?)', [staffId, loanAmount, formatDate(loanDate), status, repaymentSchedule, userId]);
...
router.put('/employee-loans/:id', auth, privilege(['loans.update']), async (req, res) => {
    ...
    const { staffId, loanAmount, loanDate, status, repaymentSchedule, userId } = req.body;
    try {
        const [result] = await db.query('UPDATE kemri_employee_loans SET staffId = ?, loanAmount = ?, loanDate = ?, status = ?, repaymentSchedule = ?, userId = ? WHERE id = ?', [staffId, loanAmount, formatDate(loanDate), status, repaymentSchedule, userId, id]);
        if (result.affectedRows === 0) {
...
router.delete('/employee-loans/:id', auth, privilege(['loans.delete']), async (req, res) => {
    const { id } = req.params;
    try {
        const [result] = await db.query('UPDATE kemri_employee_loans SET voided = 1, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Loan record not found.' });
        }
...
router.post('/monthly-payroll', auth, privilege(['payroll.create']), async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO kemri_monthly_payroll (staffId, payPeriod, grossSalary, netSalary, allowances, deductions, userId) VALUES (?, ?, ?, ?, ?, ?, ?)', [staffId, formatDate(payPeriod), grossSalary, netSalary, allowances, deductions, userId]);
...
router.put('/monthly-payroll/:id', auth, privilege(['payroll.update']), async (req, res) => {
    ...
    const { staffId, payPeriod, grossSalary, netSalary, allowances, deductions, userId } = req.body;
    try {
        const [result] = await db.query('UPDATE kemri_monthly_payroll SET staffId = ?, payPeriod = ?, grossSalary = ?, netSalary = ?, allowances = ?, deductions = ?, userId = ?, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [staffId, formatDate(payPeriod), grossSalary, netSalary, allowances, deductions, userId, id]);
        if (result.affectedRows === 0) {
...
router.delete('/monthly-payroll/:id', auth, privilege(['payroll.delete']), async (req, res) => {
    const { id } = req.params;
    try {
        const [result] = await db.query('UPDATE kemri_monthly_payroll SET voided = 1, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Payroll record not found.' });
        }
...
router.post('/employee-dependants', auth, privilege(['dependants.create']), async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO kemri_employee_dependants (staffId, dependantName, relationship, dateOfBirth, userId) VALUES (?, ?, ?, ?, ?)', [staffId, dependantName, relationship, formatDate(dateOfBirth), userId]);
...
router.put('/employee-dependants/:id', auth, privilege(['dependants.update']), async (req, res) => {
    ...
    const { staffId, dependantName, relationship, dateOfBirth, userId } = req.body;
    try {
        const [result] = await db.query('UPDATE kemri_employee_dependants SET staffId = ?, dependantName = ?, relationship = ?, dateOfBirth = ?, userId = ? WHERE id = ?', [staffId, dependantName, relationship, formatDate(dateOfBirth), userId, id]);
        if (result.affectedRows === 0) {
...
router.delete('/employee-dependants/:id', auth, privilege(['dependants.delete']), async (req, res) => {
    const { id } = req.params;
    try {
        const [result] = await db.query('UPDATE kemri_employee_dependants SET voided = 1 WHERE id = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Dependant record not found.' });
        }
...
router.post('/employee-terminations', auth, privilege(['terminations.create']), async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO kemri_employee_terminations (staffId, exitDate, reason, exitInterviewDetails, userId) VALUES (?, ?, ?, ?, ?)', [staffId, formatDate(exitDate), reason, exitInterviewDetails, userId]);
...
router.put('/employee-terminations/:id', auth, privilege(['terminations.update']), async (req, res) => {
    ...
    const { staffId, exitDate, reason, exitInterviewDetails, userId } = req.body;
    try {
        const [result] = await db.query('UPDATE kemri_employee_terminations SET staffId = ?, exitDate = ?, reason = ?, exitInterviewDetails = ?, userId = ?, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [staffId, formatDate(exitDate), reason, exitInterviewDetails, userId, id]);
        if (result.affectedRows === 0) {
...
router.delete('/employee-terminations/:id', auth, privilege(['terminations.delete']), async (req, res) => {
    const { id } = req.params;
    try {
        const [result] = await db.query('UPDATE kemri_employee_terminations SET voided = 1, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Termination record not found.' });
        }
...
router.post('/employee-bank-details', auth, privilege(['bank_details.create']), async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO kemri_employee_bank_details (staffId, bankName, accountNumber, branchName, isPrimary, userId) VALUES (?, ?, ?, ?, ?, ?)', [staffId, bankName, accountNumber, branchName, isPrimary, userId]);
...
router.put('/employee-bank-details/:id', auth, privilege(['bank_details.update']), async (req, res) => {
    ...
    const { staffId, bankName, accountNumber, branchName, isPrimary, userId } = req.body;
    try {
        const [result] = await db.query('UPDATE kemri_employee_bank_details SET staffId = ?, bankName = ?, accountNumber = ?, branchName = ?, isPrimary = ?, userId = ?, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [staffId, bankName, accountNumber, branchName, isPrimary, userId, id]);
        if (result.affectedRows === 0) {
...
router.delete('/employee-bank-details/:id', auth, privilege(['bank_details.delete']), async (req, res) => {
    const { id } = req.params;
    try {
        const [result] = await db.query('UPDATE kemri_employee_bank_details SET voided = 1, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Bank details not found.' });
        }
...
router.post('/employee-memberships', auth, privilege(['memberships.create']), async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO kemri_employee_memberships (staffId, organizationName, membershipNumber, startDate, endDate, userId) VALUES (?, ?, ?, ?, ?, ?)', [staffId, organizationName, membershipNumber, formatDate(startDate), formatDate(endDate), userId]);
...
router.put('/employee-memberships/:id', auth, privilege(['memberships.update']), async (req, res) => {
    ...
    const { staffId, organizationName, membershipNumber, startDate, endDate, userId } = req.body;
    try {
        const [result] = await db.query('UPDATE kemri_employee_memberships SET staffId = ?, organizationName = ?, membershipNumber = ?, startDate = ?, endDate = ?, userId = ? WHERE id = ?', [staffId, organizationName, membershipNumber, formatDate(startDate), formatDate(endDate), userId, id]);
        if (result.affectedRows === 0) {
...
router.delete('/employee-memberships/:id', auth, privilege(['memberships.delete']), async (req, res) => {
    const { id } = req.params;
    try {
        const [result] = await db.query('UPDATE kemri_employee_memberships SET voided = 1, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Membership record not found.' });
        }
...
router.post('/employee-benefits', auth, privilege(['benefits.create']), async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO kemri_employee_benefits (staffId, benefitName, enrollmentDate, status, userId) VALUES (?, ?, ?, ?, ?)', [staffId, benefitName, formatDate(enrollmentDate), status, userId]);
...
router.put('/employee-benefits/:id', auth, privilege(['benefits.update']), async (req, res) => {
    ...
    const { staffId, benefitName, enrollmentDate, status, userId } = req.body;
    try {
        const [result] = await db.query('UPDATE kemri_employee_benefits SET staffId = ?, benefitName = ?, enrollmentDate = ?, status = ?, userId = ?, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [staffId, benefitName, formatDate(enrollmentDate), status, userId, id]);
        if (result.affectedRows === 0) {
...
router.delete('/employee-benefits/:id', auth, privilege(['benefits.delete']), async (req, res) => {
    const { id } = req.params;
    try {
        const [result] = await db.query('UPDATE kemri_employee_benefits SET voided = 1, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Benefit record not found.' });
        }
...
router.delete('/assigned-assets/:id', auth, privilege(['assets.delete']), async (req, res) => {
    const { id } = req.params;
    try {
        const [result] = await db.query('UPDATE kemri_assigned_assets SET voided = 1, updatedAt = CURRENT_TIMESTAMP WHERE id = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Asset assignment not found.' });
        }
...
router.get('/public-holidays', auth, privilege(['holiday.read']), async (req, res) => {
    try {
        const [rows] = await db.query('SELECT * FROM kemri_public_holidays WHERE voided = 0 ORDER BY holidayDate DESC');
        res.json(rows);
    } catch (err) {
...
router.delete('/public-holidays/:id', auth, privilege(['holiday.delete']), async (req, res) => {
    const { id } = req.params;
    try {
        const [result] = await db.query('UPDATE kemri_public_holidays SET voided = 1 WHERE id = ?', [id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Public holiday not found.' });
        }
