...
if docker ps | grep -q mysql_db; then
    ...
    docker exec -i mysql_db mysql -u "$DB_USER" -p"$DB_PASS" "$DB_NAME" < api/migrations/create_citizen_proposals_table.sql
    docker exec -i mysql_db mysql -u "$DB_USER" -p"$DB_PASS" "$DB_NAME" < api/migrations/create_county_proposed_projects_table.sql
    docker exec -i mysql_db mysql -u "$DB_USER" -p"$DB_PASS" "$DB_NAME" < api/migrations/create_project_announcements_table.sql
    docker exec -i mysql_db mysql -u "$DB_USER" -p"$DB_PASS" "$DB_NAME" < api/migrations/seed_sample_data.sql
    
    echo ""
    echo "Migrations completed!"
    echo "Verifying tables..."
    docker exec -i mysql_db mysql -u "$DB_USER" -p"$DB_PASS" "$DB_NAME" -e "SHOW TABLES LIKE 'citizen_proposals'; SHOW TABLES LIKE 'county_proposed_projects'; SHOW TABLES LIKE 'project_announcements';"
else
